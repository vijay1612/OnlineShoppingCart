...
import static com.gmail.vijay.ecommerce.util.TestConstants.*;
...
@Sql(value = { "/sql/create-perfumes-before.sql", "/sql/create-user-before.sql",
                "/sql/create-orders-before.sql" }, executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
@Sql(value = { "/sql/create-orders-after.sql", "/sql/create-user-after.sql",
                "/sql/create-perfumes-after.sql" }, executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
public class UserControllerTest {
        ...
        public void changePassword_ReturnInputErrors() throws Exception {
                mockMvc.perform(post(PathConstants.USER + "/change/password")
                                ...
                                .andExpect(status().isOk())
                                .andExpect(view().name(Pages.USER_PASSWORD_RESET))
                                .andExpect(model().attribute("passwordError",
                                                is(ErrorMessage.PASSWORD_CHARACTER_LENGTH)))
                                .andExpect(model().attribute("password2Error",
                                                is(ErrorMessage.PASSWORD2_CHARACTER_LENGTH)));
        }
